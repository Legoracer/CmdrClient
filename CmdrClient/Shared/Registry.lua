...
function Registry:RegisterLibrary(libraryScript)
	local lib = require(libraryScript)
	...
	self.Libraries[libraryScript.Name] = lib
end

function Registry:RegisterLibrariesIn(container)
	for _, libraryScript in pairs(container:GetChildren()) do
		if libraryScript:IsA("ModuleScript") then
			self:RegisterLibrary(libraryScript)
		else
			self:RegisterLibrariesIn(libraryScript)
		end
	end
end
